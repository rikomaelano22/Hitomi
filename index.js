...
function title() {
	...
	console.log(chalk.yellow(`\n ${chalk.green('[ Created By Christian ID ]')}\n\n${chalk.yellow('[ Thanks To ]')}\n${chalk.yellow('• Irfan / Riyan')}\n${chalk.yellow('• Christian ID')}\n${chalk.yellow('• Nc Tech')}\n`))
}
          ...
          if (isWelcome) {
            try {
              ...
              for (let i of data.participants) {
                ...
                if (data.action == "add") {
                  conn.sendMessage(data.id, { image: { url: pp_user }, caption: `Hallo @${i.split("@")[0]} 👋\nSelamat Datang Di Grup *${metadata.subject}*`, mentions: [i] })
                } else if (data.action == "remove") {
                  conn.sendMessage(data.id, { image: { url: pp_user }, caption: `Good Bye @${i.split("@")[0]} 👋`, mentions: [i] })
                }
              }
            } catch (e) {
              console.log(e)
